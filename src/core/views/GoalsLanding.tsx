...
import { 
  ...
  Brain, Check, Star, Terminal, Radio, ShieldCheck, GraduationCap, Lock
} from 'lucide-react';
...
export const GoalsLanding: React.FC<GoalsLandingProps> = ({ onOpenAuth, onSelectExperience }) => {
  ...
  const SLIDES: MiniAppSlide[] = [
    ...
    },
    {
      ...
      btnBg: 'bg-slate-800 hover:bg-slate-700 text-white font-bold',
  ...
  return (
    <div className="relative w-full text-slate-100 font-display">
      ...
      {/* Contenedor Responsivo Full-Width Elegante */}
      <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">

        {/* ========================================================================= */}
        {/* BLOQUE 1: HERO ELEGANTE Y PROFESIONAL CON PRESENTACIÓN OFICIAL DE GOALS */}
        {/* ========================================================================= */}
        <section 
          id="section-snap-1" 
        ...
        >
          <div className="w-full h-full bg-slate-900/90 border border-slate-800 rounded-3xl p-4 sm:p-6 shadow-2xl relative overflow-hidden flex flex-col justify-between items-center text-center">
            
            {/* Header del Hero con Logo Limpio y Slogan */}
            <div className="flex flex-col items-center gap-3 z-10 max-w-xl my-auto">
              
              <div className="flex items-center gap-3">
                <img
                  src="/goals_platform_logo.png"
                  alt="GOALS Platform Logo"
                  className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl border border-slate-700 shadow-md object-cover"
                />
                <div className="text-left">
                  <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-white leading-none">
                    GOALS
                  </h1>
                  <p className="text-xs sm:text-sm text-amber-400 font-semibold tracking-wide">
                    Tu Plataforma Educativa IA
                  </p>
                </div>
              </div>

              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-800/80 border border-slate-700 text-slate-300 text-[10px] font-bold uppercase tracking-widest mt-1">
                <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                <span>Inteligencia Educativa Adaptativa</span>
              </div>
              
              <p className="text-xs sm:text-sm text-slate-300 max-w-lg mx-auto leading-relaxed font-sans">
                Una solución pedagógica avanzada donde la resolución de cuadernos manuscritos, la práctica de conversación por voz, el laboratorio tridimensional de astrofísica y el análisis de evidencia científica conviven en un solo entorno unificado.
              </p>
            </div>

            {/* PANEL INTERACTIVO DE NAVEGACIÓN POR MINI APPS */}
            <div className="w-full my-auto py-1 relative z-10">
              
              <div className="relative max-w-2xl mx-auto bg-slate-950/80 border border-slate-800 rounded-2xl p-3 shadow-inner">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 relative z-10">
                  ...
                  <div 
                    ...
                    className="group cursor-pointer bg-slate-900/90 border border-slate-800 hover:border-emerald-500/60 p-3 rounded-xl text-center transition-all hover:scale-102 shadow-sm"
                  >
                    <div className="w-8 h-8 mx-auto rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center justify-center mb-1.5">
                      <GraduationCap className="w-4 h-4" />
                    </div>
                    <div className="font-bold text-xs text-white">Escuela IA</div>
                    <div className="text-[9px] text-slate-400 font-medium">Tutor OCR</div>
                  </div>

                  {/* Nodo 2: Idiomas */}
                  <div 
                    onClick={() => onSelectExperience('languages')}
                    className="group cursor-pointer bg-slate-900/90 border border-slate-800 hover:border-indigo-500/60 p-3 rounded-xl text-center transition-all hover:scale-102 shadow-sm"
                  >
                    <div className="w-8 h-8 mx-auto rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 flex items-center justify-center mb-1.5">
                  ...
                  <div 
                    ...
                    className="group cursor-pointer bg-slate-900/90 border border-slate-800 hover:border-slate-600 p-3 rounded-xl text-center transition-all hover:scale-102 shadow-sm"
                  >
                    <div className="w-8 h-8 mx-auto rounded-lg bg-slate-800 text-slate-300 flex items-center justify-center mb-1.5">
                      <ShieldCheck className="w-4 h-4" />
                    </div>
                    <div className="font-bold text-xs text-white">Verifica</div>
                    <div className="text-[9px] text-slate-400 font-medium">Rigor Científico</div>
                  </div>
                ...
                <div className="mt-2.5 pt-2 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-400 px-2">
                  <span className="flex items-center gap-1.5 font-semibold text-slate-300"><Zap className="w-3.5 h-3.5 text-amber-400" /> Progreso Sincronizado</span>
                  <span className="text-[9.5px] text-slate-500 font-medium uppercase tracking-wider">Acceso Unificado</span>
                  <span className="flex items-center gap-1.5 font-semibold text-slate-300"><Flame className="w-3.5 h-3.5 text-amber-500" /> Racha Diaria</span>
                </div>

              </div>
            </div>

            {/* Acciones e Indicador de Navegación */}
            <div className="z-10 w-full space-y-2">
              <div className="flex justify-center items-center gap-3">
                <button
                  onClick={() => onOpenAuth('signup')}
                  className="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs uppercase tracking-wider transition-all shadow-lg active:scale-95 flex items-center gap-2 cursor-pointer"
                >
              ...
              <button 
                ...
                className="inline-flex items-center gap-1 text-[10px] font-semibold text-slate-400 hover:text-white transition-colors pt-0.5"
              >
        ...
        {/* ========================================================================= */}
        {/* BLOQUE 2: CARRUSEL DE MINI APPS (ELEGANTE Y ESTRUCTURADO) */}
        {/* ========================================================================= */}
        <section 
          id="section-snap-2" 
          className="h-[calc(100vh-89px)] min-h-[480px] snap-start flex flex-col justify-center py-1 relative overflow-hidden"
        >
          <div className="w-full h-full bg-slate-900/90 border border-slate-800 rounded-3xl p-4 sm:p-5 flex flex-col justify-between relative overflow-hidden transition-all duration-300 shadow-xl">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-2.5 py-0.5 rounded-full text-[9.5px] font-bold border ${currentSlide.badgeBg}`}>
                ...
                {SLIDES.map((s, idx) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${
                      currentSlideIndex === idx ? 'w-6 bg-amber-400' : 'w-2 bg-slate-700 hover:bg-slate-500'
                    }`}
                    title={`Ver ${s.name}`}
                  />
                ))}
            ...
            {/* Vista Previa Visual */}
            <div className="relative w-full flex-1 my-2 bg-slate-950 rounded-2xl overflow-hidden group border border-slate-800">
              <img
                src={currentSlide.image}
                alt={currentSlide.heading}
                className="w-full h-full object-cover opacity-80 group-hover:opacity-95 transition-all duration-500"
              />
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/60 to-transparent p-4 flex flex-col justify-end">
                <h3 className="font-bold text-sm sm:text-base text-white drop-shadow">
            ...
            <div className="space-y-2 pt-1.5 border-t border-slate-800">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {currentSlide.bullets.slice(0, 2).map((bullet, idx) => (
                  <div key={idx} className="bg-slate-950/60 border border-slate-800 rounded-xl p-2 flex items-start gap-2 text-xs text-slate-300">
        ...
        <section 
          ...
          className="h-[calc(100vh-89px)] min-h-[480px] snap-start flex flex-col justify-center py-1 relative overflow-hidden"
        >
          <div className="w-full h-full bg-slate-900/90 border border-slate-800 rounded-3xl p-5 sm:p-6 flex flex-col justify-between items-center text-center shadow-2xl">
            
            <div className="space-y-1.5">
              <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-slate-300 text-[10px] font-bold uppercase tracking-widest">
                <Brain className="w-3.5 h-3.5 text-amber-400" />
                <span>Enfoque Pedagógico Integrado</span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-black text-white">¿Cómo Funciona la Metodología GOALS?</h2>
              <p className="text-xs text-slate-400 max-w-md mx-auto">Reemplaza el estudio memorístico pasivo por una interacción guiada adaptada al ritmo del estudiante.</p>
            </div>

            {/* Grid de 3 Pilares Fundamentales */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full my-auto">
              
              <div className="bg-slate-950/80 border border-slate-800 rounded-2xl p-4 text-left space-y-2 hover:border-slate-700 transition-all">
                <div className="w-9 h-9 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400">
                  <Brain className="w-5 h-5" />
                </div>
                <h3 className="font-bold text-sm text-white">Tutoría Adaptativa Individual</h3>
                <p className="text-xs text-slate-300 leading-relaxed">
                  Identifica patrones de duda en tiempo real. Ajusta el nivel de las pistas y explicaciones según el desempeño registrado en cada sesión.
                </p>
              </div>
              ...
              <div className="bg-slate-950/80 border border-slate-800 rounded-2xl p-4 text-left space-y-2 hover:border-slate-700 transition-all">
                ...
                <h3 className="font-bold text-sm text-white">Visualización Tridimensional</h3>
                <p className="text-xs text-slate-300 leading-relaxed">
              ...
              <div className="bg-slate-950/80 border border-slate-800 rounded-2xl p-4 text-left space-y-2 hover:border-slate-700 transition-all">
                ...
                <p className="text-xs text-slate-300 leading-relaxed">
                  Fomenta la constancia diaria acumulando puntos de experiencia y manteniendo la racha activa a través de los cuatro módulos educativos.
                </p>
              </div>
        ...
        >
          <div className="w-full h-full bg-slate-900/90 border border-slate-800 rounded-3xl p-5 sm:p-6 flex flex-col justify-between items-center text-center shadow-2xl">
            ...
            <div className="space-y-1.5">
              ...
              <h2 className="text-2xl sm:text-3xl font-black text-white">Arquitectura Tecnológica</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full my-auto">
              ...
              <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 text-left space-y-2">
              ...
              <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 text-left space-y-2">
                ...
                <p className="text-xs text-slate-300 leading-relaxed">
                  Constantes físicas verificadas, datos orbitales exactos y contraste automático de veracidad frente a la desinformación.
                </p>
              </div>
              ...
              <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 text-left space-y-2">
                ...
                <p className="text-xs text-slate-300 leading-relaxed">
                  Simulación de cuerpos celestes con shaders procedimentales fluidos y navegación en tiempo real sin instalaciones externas.
                </p>
              </div>
        ...
        {/* ========================================================================= */}
        {/* BLOQUE 5: LLAMADA A LA ACCIÓN FINAL (CTA ELEGANTE Y DIRECTO) */}
        {/* ========================================================================= */}
        <section 
          id="section-snap-5" 
          className="h-[calc(100vh-89px)] min-h-[480px] snap-start flex flex-col justify-center py-1 relative overflow-hidden"
        >
          <div className="w-full h-full bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 flex flex-col justify-between items-center text-center shadow-2xl">
            ...
            <div className="space-y-3 max-w-lg my-auto">
              ...
              <div className="pt-3 flex flex-col sm:flex-row justify-center gap-3">
                <button
                  onClick={() => onOpenAuth('signup')}
                  className="w-full sm:w-auto px-8 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs uppercase tracking-wider transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2 cursor-pointer"
                >
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-6 py-3 rounded-xl bg-slate-950 border border-slate-700 hover:border-white text-white font-bold text-xs transition-all active:scale-95 cursor-pointer"
                >
                  Iniciar Sesión
                </button>
              </div>
